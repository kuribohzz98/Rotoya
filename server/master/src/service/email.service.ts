...
export class EmailService {
    ...
    async sendMail(listTo: string[], subject: string, message: string): Promise<any> {
    ...
    getOptionsSendMail(listTo: string[], subject: string, message: string): any {
        if (!listTo || !listTo.length) throw new Error(`'to' in mailer is underfine`);
        ...
        return {
            from: 'Rotoya' + this.configService.get('MAIL_USER') || `Rotoya <justen.jacobson98@ethereal.email>`,
            to: listTo.join(', '),
            subject,
            text: message
        }
