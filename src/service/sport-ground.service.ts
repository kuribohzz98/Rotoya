...
@Injectable()
export class SportGroundService extends BaseService<
  SportGroundRepository,
  SportGround,
  SportGroundAttribute,
  SportGroundDto
> {
  constructor(private readonly sportGroundRepository: SportGroundRepository) {
  ...
  async getSportGround(opts: TypeQueryGetSportCenter) {
    ...
    sportGround.raw.map(raw => {
      const timeSlot = result.sportGroundTimeSlots.find(
        sgTimeSlot =>
          sgTimeSlot.id ==
          raw[`${this.sportGroundRepository.models.sport_ground_time_slot}_id`],
      );
      if (timeSlot) {
        if (raw[`count`]) {
          const saved = timeSlot.bookeds.find(
            booked =>
              ...
              new Date(
                raw[`${this.sportGroundRepository.models.booking}_bookingDate`],
              ).getTime(),
          );
          !saved &&
        ...
        if (
          raw[
            `${this.sportGroundRepository.models.sport_center_equipment_booking}_id`
          ]
        ) {
          const sceBooking = this.sportGroundRepository.models
