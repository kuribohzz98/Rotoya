...
@Injectable()
export class RequestCoOperateService extends BaseService<
  RequestCoOperateRepository,
  RequestCoOperate,
  RequestCoOperateAttribute
> {
  constructor(
  ...
  async update(
    id: number,
    opts: RequestCoOperateAttribute,
    ...
    if (opts.status == ECoOperateStatus.REJECTED) {
      await this.emailService.sendMail(
        ...
        opts.note ||
          `Chúng tôi rất lấy làm tiếc khi phải từ chối yêu cầu của bạn. Mong một ngày không xa có thể được cộng tác với bạn thật sự.`,
      );
    }
