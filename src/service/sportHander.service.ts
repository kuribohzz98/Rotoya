...
export class SportHanderService {
  ...
  getSportCenterTimeBlank$(
    ...
    return of(this.caculateFourPoint(query)).pipe(
      mergeMap((data: TypePointFourDirection) =>
        from(
          this.sportCenterService.getSportCenterBySlotTimeBlank(
            query.sportId,
            +query.time,
            {
              ...query,
  ...
  getSportCenterByGeoLocation$(
    ...
    return of(this.caculateFourPoint(query)).pipe(
      mergeMap((data: TypePointFourDirection) =>
        from(this.sportCenterService.getSportCentersByGeolocation(query, data)),
      ),
    );
  }
  ...
  caculateFourPoint(query: TypeQueryGetSportCenters) {
    ...
    data.pointNorth = destinationPointGivenDistanceAndBearingFromStartPoint(
      query,
      0,
    );
    data.pointEast = destinationPointGivenDistanceAndBearingFromStartPoint(
      query,
      toRadian(90),
    );
    data.pointSouth = destinationPointGivenDistanceAndBearingFromStartPoint(
      query,
      toRadian(180),
    );
    data.pointWest = destinationPointGivenDistanceAndBearingFromStartPoint(
      query,
      toRadian(270),
    );
    return data;
