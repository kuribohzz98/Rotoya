...
> {
  ...
  async getPayments(opts?: OptionsQueryPayment) {
    ...
    if (opts && opts.page && opts.limit) {
      ...
      return this.getByOptions(
        options,
        ['sportCenter', 'user', 'user.userInfo'],
  ...
  async getPaymentInfo(orderId: string) {
    ...
    return this.createQueryBuilder(payment)
      ...
      .leftJoinAndMapMany(
        ...
        sce_booking,
        sce_booking,
        `${booking}.id = ${sce_booking}.bookingId`,
      )
      .leftJoinAndMapOne(
        `${sce_booking}.sportCenterEquipment`,
        sce,
        sce,
        `${sce}.id = ${sce_booking}.sportCenterEquipmentId`,
      )
      .leftJoinAndMapOne(
        `${payment}.user`,
        user,
      ...
      .leftJoinAndMapOne(
        ...
        sportEquipment,
        sportEquipment,
        `${sportEquipment}.id = ${sce}.sportEquipmentId`,
      )
      .leftJoinAndMapOne(
        `${payment}.sportCenter`,
        sportCenter,
        sportCenter,
        `${payment}.sportCenterId = ${sportCenter}.id`,
      )
      .leftJoinAndMapOne(
        `${booking}.sportGroundTimeSlot`,
        timeSlot,
        timeSlot,
        `${timeSlot}.id = ${booking}.timeSlotId`,
      )
      .leftJoinAndMapOne(
  ...
  async getPaymentByTimeSlotId(timeSlotId: number, bookingDate: string) {
    ...
    return this.createQueryBuilder(payment)
      ...
      .andWhere(`${booking}.bookingDate LIKE :bookingDate`, {
