...
export class TimeSlotRepository extends
    BaseRepository<SportGroundTimeSlot, SportGroundTimeSlotAttribute>  {
    ...
    getTimeSlotByDateBook(id: number, dateBook: string) {
        ...
        return this.createQueryBuilder(timeSlot)
            ...
            .leftJoinAndMapMany(`${timeSlot}.bookings`, qb => {
                return qb.select()
                    .addFrom<Booking>('Booking', booking)
                    .where(`${booking}.bookingDate LIKE :dateBook`, { dateBook: dateBook + '%' })
