...
export function RandomPassword(): string {
  ...
  for (var i = 0, c = 0, cl = 0, sc = 0, num = 0; i < length; i++) {
    ...
    if (random == 0 && c < 4) {
      ...
      retVal += capitalize_letter.charAt(
        Math.floor(Math.random() * capitalize_letter.length),
      );
      cl++;
    } else if (random == 2 && sc < 2) {
      retVal += special_character.charAt(
        Math.floor(Math.random() * special_character.length),
      );
      sc++;
    } else {
      if (num < 2) {
