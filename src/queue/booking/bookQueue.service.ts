    ...
    async handlerJobInQueue(data: BookSportGround) {
        ...
        await Promise.all(
            data.bookDatas.map(async bookData => {
                ...
                try {
                    const result = await this.bookingService.checkTimeSlot(bookData.timeSlotId, bookDate);
...
export class BookQueueService implements OnModuleInit {
    ...
    public subject$: BehaviorSubject<BookSportGround> = new BehaviorSubject<BookSportGround>(null);
    ...
    watchAddJob() {
        this.subject$.subscribe(data => {
            ...
            if (!this.bookQueue[data.sportCenterId]) {
                ...
                this.bookQueue = { ...this.bookQueue, [data.sportCenterId]: bookQueueDataTemp };
                return;
            }
